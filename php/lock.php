...
if (isset($_SESSION['id_Admin'])&&(isset($_SESSION['admin_name']))) {
    ...
    echo "<!DOCTYPE html>
          ...
          <head>
                <meta charset='UTF-8'>
                <link rel='icon' href='../res/logo.ico' type='image/png'>
                <link rel='shortcut icon' href='../res/logo.ico' type='img/x-icon'>
                <title>Вход AS Lume</title>
          </head>
          <body>
                <div class='container'> 
                    <div class='entry_form'> 
                        <div class='header'><h1>adminLume</h1><a href='http://lume.by/'> </a></div> 
    ...
    if (isset($_POST['admin_login']) && isset($_POST['admin_password'])){
        $result =sql_select("SELECT `id_Admin`, `admin_name`,`admin_login`,`admin_password` FROM `admins` 
                            WHERE `admin_login` = '" . mysqli_real_escape_string($db, $_POST['admin_login']) . "' 
                            AND `admin_password` = '" . mysqli_real_escape_string($db, $_POST['admin_password']) . "' LIMIT 1") or die("Ошибка идентификации" . mysqli_error($db));
